...
ApplicationWindow {
    ...
    function getEnergyLoss(radius, tracks, cals){
		...
		else if (radius > siliconDetector.stopRadius/2 && radius < calorimeters.stopRadius/2 && cals) return 0.9;
		else if (radius < calorimeters.stopRadius/2 && radius < muonChamber.stopRadius/2 && tracks) return 0.999;
		else return 1.;
    }
